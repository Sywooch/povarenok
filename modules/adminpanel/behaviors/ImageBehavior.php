 ...
 * Time: 12:45
 */
namespace app\modules\adminpanel\behaviors;
...
class ImageBehavior extends Behavior
{
    ...
    public function createImage()
    {
        ...
        if($model->validate()):
            ...
            $modelPhoto->file       = $this->imageData['imagePath'].$fileName;
            $modelPhoto->file_small = $this->imageData['imagePath'].$smallFileName;
            ...
            try {
                if($modelPhoto->save() && $commit):
                    FileHelper::createDirectory(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'], $mode = 509);
                    if($model->image->saveAs(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$fileName)):
                        $image = Image::getImagine();
                        $newImage = $image->open(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$fileName);
                        ...
                        if($newImage->crop(
                            ...
                            ->save(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$fileName)):

                            $newImage = $image->open(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$fileName);
                            $newImage->thumbnail(new Box($this->imageData['imageSmallWidth'], $this->imageData['imageSmallHeight']))
                                ->save(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$smallFileName);
                            $transaction->commit();
                        endif;
                    endif;
    ...
    public function updateImage()
    {
        ...
        if($model->validate()):
            ...
            try {
                ...
                $this->deleteImageFile(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$fileName);
                $this->deleteImageFile(\Yii::getAlias($this->imageData['baseUrl']).$this->imageData['imagePath'].$smallFileName);
                $transaction->rollBack();
            }
        else:
            \Yii::$app->session->set('error', $model->errors['image']['0']);
